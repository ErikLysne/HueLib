 ...
 * \endcode
 *
 * HueBridge needs access to \c QNetworkAccessManager. By default, it will create one internally, but if this
 * resource is required elsewhere, it can be created externally and a reference to HueBridge can be provided.
 * Use a third argument to provide a pointer to a \c QNetworkAccessManager.
 * \code
...
/*!
 * Destroys the HueBridge. If a \e QNetworkAccessManager is created internally, it will be destroyed as well.
 */
HueBridge::~HueBridge()
{
 ...
 *
 * The first time link is called in an application, the user will be prompted to press the link button on the Hue bridge.
 ...
 *
 * link always checks if a \e username.txt file exists in the \e .huelib folder before attempting to create a new user, so calling
 * link immediately after constructing HueBridge means the username never has to be set explicitly.
 *
